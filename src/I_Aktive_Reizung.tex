\chapter{Eröffnungen} \label{ch:Eröffnungen}

\begin{bids}
	\hyperref[ch:1c]{1\c\rs}	& 11\rplus, 2\rplus \c \\
							    & \subbids{
		        				a. & 12-14 balanced, ohne 5er \M \\
						        b. & 11\rplus, Einfärber/Zweifärber, längste Farbe \c \\
							    c. & 12\rplus, 4414} \\
	\hyperref[ch:1d]{1\d\rs}	& 11\rplus, 2\rplus \d \\
							    & \subbids{
							    a. & 18-20 balanced, ohne 5er \M \\
							    b. & 11\rplus, Einfärber/Zweifärber, längste Farbe \d \\
							    c. & 12\rplus, 4441, 4144, 1444 \\
							    d. & 11-15, 5\c{} 4\d, außer wenn \c s zu schön / \d s zu schwach} \\
	\hyperref[ch:1M]{1\M}       & 11\rplus, 5\rplus \M \\
						        & \subbids{
							    a. & 12-14 / 18-20 balanced, 5\M \\
							    b. & 11\rplus, Einfärber/Zweifärber, längste Farbe \M} \\
	\hyperref[ch:1SA]{1SA} 	    & 15-17 balanced, 5er \M{} möglich \\
	\hyperref[ch:2c]{2\c\rs}	& a. gf balanced und unbalanced Hände \\
								& b. 23-24	balanced, 5er \M{} möglich \\
	\hyperref[ch:2d]{2\d\ra}	& schwach (\tbd{in 1./2. Hand 8-10}), \M-Zweifärber \\
	\hyperref[ch:2M]{2\M\rs}	& 6\rplus\M, schwach \\
	\hyperref[ch:2SA]{2SA\rka}	& 21-22 balanced, 5er \M{} möglich \\
	\hyperref[ch:3x]{3x}        & 7\rplus x, schwach
\end{bids}

\chapter{1\c- Eröffnung} \label{ch:1c}

Hände, die man mit 1\c{} eröffnet, fallen grob in zwei Kategorien: Balanced oder Unbalanced.

Alle Balanced Hände mit 12-14 Punkten und ohne 5er-Oberfarbe werden mit 1\c{} eröffnet, unabhängig davon, welche Unterfarbe länger ist.
Wenn man 1\c{} eröffnet und im Rebid eine Balanced Hand zeigt, hat man immer 12-14 Punkte,
andere Punktespannen für Balanced Hände werden stattdessen 1\d, 1SA, 2SA oder 2\c{} eröffnet.

Unbalanced Hände, die mit 1\c{} eröffnet werden sind entweder \c-Einfärber, Zweifärber mit \c{} als längste Farbe, oder 4414-Shape.

\section{Antworten auf 1\c\rs} \label{sec:1c}

Auf 1er-Stufe sind die Antworten Transfers: 1\d\ra{} zeigt 4\rplus\h, 1\h\ra{} zeigt 4\rplus\s, 1\s\ra{} verneint 4er-OF und \glqq transferiert\grqq{} auf 1SA.
Mit der Balanced Hand führt man den Transfer immer aus (auch mit Fit),
mit allen anderen Händen führt man den Transfer niemals aus, sondern beschreibt seine Hand weiter.

Die Antworten 2\c{} und 2\d{} sind künstlich. 2\c\ra{} ist ein gameforcing Relay, 2\d\ra{} zeigt 5\rplus\s{} 4\rplus\h.

Prioritäten bei den Antworten:
\begin{itemize}
	\item Mit gf Händen ohne 5er-OF reizt man 2\c\ra{} (4er-OF zeigt man später)
	\item Mit 44 in den Oberfarben ohne gf reizt man 1\d\ra
	\item Mit 5\rplus\s{} 4\rplus\h{} antwortet man ohne gf 2\d\ra, mit gf 1\h\ra
\end{itemize}

\tbd{Alternative: Bei gf Hände mit 4er-Oberfarben diese immer auf 1er-Stufe reizen, 2\c{} verneint dann 4er-Oberfarbe}

Antworten auf 1\c\rs:
\nopagebreak

\begin{bids}
	Pass    & 0-5 (viele 3-5 Hände eignen sich aber für Transfers!) \\
	1\d\ra  & 3\rplus, 4\rplus\h \\
	        & mit 3-5 sollte man 5er\h{} oder min. 43 in \d\s{} haben \\
	        & mit gf und nur 4er\h{} stattdessen 2\c\ra{} reizen \\
	        & \ensuremath{\rightarrow} \ref{subsec:1c1d} \\
	1\h\ra  & 3\rplus, 4\rplus\s \\
			& mit 3-5 sollte man 5er\s{} oder min. 43 in \d\h{} haben \\
			& mit gf und nur 4er\s{} stattdessen 2\c\ra{} reizen \\
			& \ensuremath{\rightarrow} \ref{subsec:1c1H} \\
	1\s\ra  & a. 6-7 balanced, keine 4\rplus\M \\
			& b. 6-11 unbalanced, keine 4\rplus OF (Notgebot) \\
			& \ensuremath{\rightarrow} \ref{subsec:1c1S} \\
	1SA     & 8-11 balanced, keine 4\rplus\M{} \ensuremath{\rightarrow} \ref{subsec:1c1SA} \\
	2\c\ra  & gf Relay, keine 5\rplus\M{} \ensuremath{\rightarrow} \ref{subsec:1c2c} \\
	2\d\ra  & 3-11, 5\rplus\s{} 4\rplus\h{} \ensuremath{\rightarrow} \ref{subsec:1c2d} \\
	2\M     & schwach, 6\rplus\M{} \ensuremath{\rightarrow} \ref{subsec:1c2M} \\
	2SA     & 11-12, 4333 ohne 4er OF \ensuremath{\rightarrow} 3\c{} to play; Rest gf \\
	3\m\ra  & 9-11, 6\rplus\m{} (mind. KB10xxx), ohne 4er OF \ensuremath{\rightarrow} alles außer pass ist gf \\
	3\M     & schwach, 7\rplus\M{} \ensuremath{\rightarrow} wie nach 3M Eröffnung \\
	3SA     & 13-15, 4333 ohne 4er \M
\end{bids}

\subsection{1\c\rs{} 1\d\ra} \label{subsec:1c1d}

Transfers in eine Oberfarbe führt der 1\c-Eröffner mit der Balanced Hand immer aus. Daher stehen im Rebid 1SA und 2SA als künstliche Gebote zur Verfügung.
1SA\ra{} zeigt einen Einfärber (entweder schwach mit 3er-Anschluss oder stark), 2SA\ra{} ist eine starke Hebung der Oberfarbe. Dies ermöglicht, dass 3\c{} und 4\M{} jetzt immer extreme Verteilungen zeigen und 3SA als to play zur Verfügung steht.

\begin{bids}
	Pass    & verboten \\
	1\h\ra  & 12-14 balanced, 2-4\h \ensuremath{\rightarrow} \ref{subsubsec:1c1d1H} \\
	1\s     & 11\rplus, Zweifärber 5\rplus\c{} 4\rplus\s, fast nie zu passen \ensuremath{\rightarrow} \ref{subsubsec:1c1d1S} \\
	1SA\ra  & a. 11-15, Einfärber 6\rplus\c{} mit 3er\h \\
	        & b. 16\rplus, starker Einfärber mit 6\rplus\c \\
	        & \ensuremath{\rightarrow} \ref{subsubsec:1c1d1SA} \\
	2\c\ra  & a. 11-15, Zweifärber 5\rplus\c{} 4\rplus\d{} ohne 3er\h \\
	        & b. 11-15, Einfärber 6\rplus\c{} ohne 3er\h \\
	        & \ensuremath{\rightarrow} \ref{subsubsec:1c1d2c} \\
	2\d     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\d, fast nie zu passen \ensuremath{\rightarrow} \ref{subsubsec:1c1d2d} \\
	2\h     & 11-15, 3\rplus\h{} (3er fit kann nur 1345 sein, meist 4er) \ensuremath{\rightarrow} \ref{subsubsec:1c1d2H} \\
	2\s\rka & 10\rplus, 6\rplus\c{} 5\rplus\s \ensuremath{\rightarrow} \ref{subsubsec:1c1d2S} \\
	2SA\ra  & 19\rplus, 4\rplus\h \ensuremath{\rightarrow} \ref{subsubsec:1c1d2SA} \\
	3\c\rka & 13-15, 7\rplus\ \ensuremath{\rightarrow} alles außer pass ist gf, neue Farben zeigen Stopper \\
	3\d\ra  & 19\rplus, 4\rplus\h, \d-Splinter \\
	3\s\ra  & 19\rplus, 4\rplus\h, \s-Splinter \\
	3\h     & 16-18, 4\rplus\h \\
	3SA     & to play, basierend auf semi-forcing in \c{} mit \h-Kürze \\
	4\c     & Schlemminteresse, 6\rplus\c{} 4\h \\
	4\h     & 10-12, 6\rplus\c{} 5\h
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 1\h\ra} \label{subsubsec:1c1d1H}

Eröffner hat 12-14 balanced und kann 2-4 \h s haben. Jetzt ist 2-Way-Checkback an.
2\c{} ist ein Relay, entweder um 2\d{} (bzw. 2\h, falls Eröffner 4er\h{} hat) zu spielen, oder um diverse einladende Hände zu zeigen.
2\d{} ist gameforcing und zeigt 5er\h{} (\tbd{nur 4\rplus\h, falls 1\c{} 2\c{} 4er-Oberfarbe verneint}).
2SA ist ein Relay zu 3\c. Entweder zum Spielen oder mit Schlemminteresse in \c.
\tbd{(Alternative: 2\c{} ist inv+ mit 5er\h, 2\d{} ist gf mit nur 4er\h)}

\begin{bids}
	Pass	& erlaubt (Plicht mit 5\rminus{} Punkten) \\
	1\s     & nicht einladend, 4\rplus\h{} 4\s{} \ensuremath{\rightarrow} natürlich \\
	1SA		& to play \ensuremath{\rightarrow} 2\h{} hatte 4er\h \\
	2\c\ra  & maximal inv Relay \ensuremath{\rightarrow} \ref{subsubsec:1c1d1H2c} \\
	2\d\ra  & gf, 5er\h \\
	2\h     & 5er\h{}, Mini-Einladung (nur gegenüber 4er\h{} einladend) \\
	2\s     & inv, 4\h{} 4\s \\
	2SA\ra  & Relay zu 3\c{} \ensuremath{\rightarrow} \\
			& \subbids{
			pass & wollte 3\c{} spielen \\
			Rest & SI-\c} \\
	3\m     & inv 5\h{} 5\m{} \ensuremath{\rightarrow} pass verneint 3er\h \\
	3\h     & gf, 6er\h \\
	3\s\ra  & Splinter, 6er\h \\
	3SA		& to play \\
	4\m		& Splinter, 6er\h \\
	4\h		& to play
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 1\h\ra{} 2\c\ra} \label{subsubsec:1c1d1H2c}

\begin{bids}
	2\d\ra	& kein 4er\h \\
			& \subbids{
			Pass	& will jetzt 2\d{} spielen \\
			2\h		& inv, 5er\h \\
			2\s		& inv, 5er\h{} 4er\s \\
			2SA		& inv, 4\rplus\h \\
			3\m		& inv, 4er\h{} 5\rplus\m \\
			3\h		& inv, 6er\h} \\
	2\h\ra	& 4er\h
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 1\s} \label{subsubsec:1c1d1S}

Auch hier ist Two-Way-Checkback an. Hier ist zu beachten, dass Eröffner eine größere Range an Punkten und Verteilungen haben kann. Daher darf Eröffner nach 1\c\rs{} 1\d\ra{} 1\s{} 2\c\ra{} aus der Sequenz ausbrechen, wenn er eine Hand hat, mit der er sehr unglücklich wäre, wenn Responder 2\d{} passt \tbd{in etwas geringerem Ausmaß gilt gleiches auch für 1\c\rs{} 1\d\ra{} 1\s{} 2SA\ra}.

\begin{bids}
	Pass	& erlaubt (sollte 6\rminus{} Punkte haben) \\
	1SA		& 6-10, natürlich \\
	2\c\ra	& maximal inv Relay \ensuremath{\rightarrow} \ref{subsubsec:1c1d1S2c} \\
	2\d\ra	& gf, 5er\h \\
	2\h		& inv, 6er\h \\
	2\s		& 6-10 FV, 4er\s \\
	2SA\ra  & Relay zu 3\c{} \ensuremath{\rightarrow} \\
			& \subbids{
			pass & wollte 3\c{} spielen \\
			Rest & SI-\c} \\
	3\c		& \tbd{gf, \c-Fit?} \\
	3\d		& inv, 5er\h{} 5er\d \\
	3\h		& gf, 6er\h \\
	3\s		& \tbd{gf, \s-Fit?} \\
	4\m		& Splinter, mit \s-Fit (AUCH 4\c{} !!!) \\
	4\h		& knappes gf, 7\rplus\h \\
	4\s		& Spielvorschlag (max. 1 Keycard ohne \s-Dame)
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 1\s\ra{} 2\c\ra} \label{subsubsec:1c1d1S2c}

\begin{bids}
	2\d\ra	& Normalgebot \\
			& \subbids{
				Pass	& will jetzt 2\d{} spielen \tbd{sollte das hier erlaubt sein?} \\
				2\h		& inv, 5er\h \\
				2\s		& inv, \s-Fit \\
				2SA		& inv, 4\rplus\h \\
				3\c		& inv, \c-Fit \\
				3\d		& inv, 4er\h{} 5er\d} \\
	Anderes	& \tbd{Ausbrechen mit viel Verteilung/Punkten?}
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 1SA\ra} \label{subsubsec:1c1d1SA}

Eröffner hat entweder einen schwachen Einfärber mit 3er\h{} oder einen starken Einfärber mit oder ohne 3er\h{}.

\begin{bids}
	Pass	& verboten \\
	2\c		& to play gegenüber schwachem Einfärber \ensuremath{\rightarrow} \\
			& \subbids{
			Pass 	& schwache Variante \\
			Rest 	& starke Variante, \tbd{}} \\
	2\d\ra	& inv+ Relay \ensuremath{\rightarrow} \\
			& \subbids{
			2\h 	& schwache Variante \ensuremath{\rightarrow} Weiterreizung natürlich (Gebote unterhalb 3SA sind einladend) \\
			Rest	& starke Variante, jetzt gf, so natürlich wie möglich} \\
	2\h 	& to play gegenüber schwachem Einfärber \ensuremath{\rightarrow} \\
			& \subbids{
			Pass 	& schwache Variante \\
			Rest 	& starke Variante, \tbd{}} \\
	Rest	& \tbd{}
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 2\c\ra} \label{subsubsec:1c1d2c}

\begin{bids}
	Pass	& erlaubt \\
	2\d\ra	& \tbd{INV+ 3.FF \ensuremath{\rightarrow}} \\
			& \subbids{
			2\h		& min, ohne \s-Stopper, mit Double Figur in \h \\
			2SA		& min, mit \s-Stopper \\
			3\c 	& min, ohne \s-Stopper, ohne Double Figur in \h \\
			2\s\ra 	& max (gf), 6\rplus\c{} ohne \s-Stopper, ohne Double Figur in \h \\
			3\d 	& max (gf), 5\rplus\c{} 4\d{}, mit oder ohne \s-Stopper \\
			3\h 	& max (gf), ohne \s-Stopper, mit Double Figur in \h \\
			3SA 	& max, 6\rplus\c{} mit \s-Stopper} \\
	2\h		& inv, 6er\c \\
	2\s		& gf, 5+\h{} 4+\s \\
	2SA		& inv \\
	3\c		& inv \\
	3\d 	& gf, 5+\h{} 5+\d \\
	3\h		& gf, 6er\h \\
	3SA		& to play \\
	4\h		& to play
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 2\d} \label{subsubsec:1c1d2d}

\begin{bids}
	Pass	& erlaubt (nur mit 3-5 und nur 4er\h{} und 3\rplus\d) \\
	2\h 	& 5er\c (3\rplus), rundenforcierend \ensuremath{\rightarrow} \\
			& \subbids{
			2\s\ra	& gf (19\rplus), 4.FF, 2\rminus\h{} \ensuremath{\rightarrow} nat \\
			2SA		& nf (16-18), 2\rminus\h{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
			3\c		& nf (16-18), 2\rminus\h{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
			3\h		& nf (16-18), 3er\h{} \ensuremath{\rightarrow} alles außer pass ist gf \\
			3\s\ra	& gf (19+), Splinter, 3er\h \\
			4\h		& 19+, 3er\h} \\
	2\s\ra	& gf, 4.FF \\
	2SA		& Relay (entweder 6-7 oder SI in \c) \\
			& \subbids{
			3\c		& 16-18 \ensuremath{\rightarrow} pass und 3\d sind to play, Rest SI in \c \\
			3\d		& gf, 6\rplus\c{} 5\rplus\d \\
			3\s\ra	& gf, 4.FF \\
			Rest	& gf und nat} \\
	Rest	& gf und nat
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 2\h} \label{subsubsec:1c1d2H}

\begin{bids}
	Pass	& erlaubt \\
	2\s\ra	& relay (insbes. mit SI anwenden) \\
			& \subbids{
			2SA\ra	& 4315 Typ (kommt fast nie vor, weil Eröffner dann stattdessen 1\c{} 1\d{} 1\s{} reizen konnte) \\
			3\c\ra	& 1345 Typ (auch selten, da Eröffner 1\d{} eröffnen konnte) \\
					& Auf 2SA / 3\c{} \ensuremath{\rightarrow} \\
					& \subbids{
					3\h			& inv \\
					4\h			& to play \\
					Kürze\ra	& SI-\h \\
					4\c			& \tbd{bedingte Assfrage} \\
					3SA			& to play \\
					4er-Farbe	& gf, Fit (gf, weil die Hände dann gut zusammenpassen)} \\
			3\d\ra	& max, 4er\h \\
			3\h\ra	& min, 4er\h \\
			3\s\ra	& Splinter, 4\h{} und sehr gutes Maximum \\
			3SA		& max, 4er\h{} und schlägt 3SA vor \\
			4\d		& Splinter, 4er\h{} und sehr gutes Maximum \\
			4\h		& max, 4er\h, sehr kontrollschwach} \\
	2SA		& Long Suit Trial \s \\
	3\c		& inv, nf, 4er\h, 3\rplus Fit in \c \\
	3\d		& Long Suit Trial \d \\
	3\h 	& inv, 5er\h \ensuremath{\rightarrow} ablehnen mit min und nur 3er\h \\
	3\s\ra 	& Spliter \\
	3SA		& Spielvorschlag (Eröffner darf auch mit 4er\h passen) \\
	4\c		& Splinter (Chicane) \\
	4\d		& Splinter \\
	4\h		& to play
\end{bids}

\subsubsection{1\c\rs{} 1\d\ra{} 2\s} \label{subsubsec:1c1d2S}

\tbd{}

\subsubsection{1\c\rs{} 1\d\ra{} 2SA\ra} \label{subsubsec:1c1d2SA}

\begin{bids}
	3\h		& nf, hatte nur 3-5 Punkte \\
	3\c\ra	& gf ohne SI \\
	4\h		& gf ohne SI (0 Keycards) \\
	Rest	& gf, SI \tbd{Kürzen/Cuebids, je nach Vereinbarung}
\end{bids}

\subsection{1\c\rs{} 1\h\ra} \label{subsec:1c1H}

\begin{bids}
	Pass    & verboten \\
	1\s\ra  & 12-14 balanced, 2-4\s{}  \ensuremath{\rightarrow} \ref{subsubsec:1c1H1S} \\
	1SA\ra  & a. 11-15, Einfärber 6\rplus\c{} mit 3er\s \\
			& b. 16\rplus, starker Einfärber mit 6\rplus\c \\
			& \ensuremath{\rightarrow} \ref{subsubsec:1c1H1SA} \\
	2\c\ra  & a. 11-15, Zweifärber 5\rplus\c{} 4\rplus\d/\h ohne 3er\s \\
        	& b. 11-15, Einfärber 6\rplus\c{} ohne 3er\s \\
        	& \ensuremath{\rightarrow} \ref{subsubsec:1c1H2c} \\
	2\d     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\d, fast nie zu passen \ensuremath{\rightarrow} \ref{subsubsec:1c1H2d} \\
	2\h     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\h, fast nie zu passen \ensuremath{\rightarrow} \ref{subsubsec:1c1H2H} \\
	2\s     & 11-15, 3\rplus\s{} (3er-Fit nur bei 5431-Hand-Typ, meist 4er) \ensuremath{\rightarrow} \ref{subsubsec:1c1H2S} \\
	2SA\ra  & 19\rplus, 4\rplus\s \ensuremath{\rightarrow} \ref{subsubsec:1c1H2SA} \\
	3\c\rka & 13-15, 7\rplus\c \ensuremath{\rightarrow} alles außer pass ist gf, neue Farben zeigen Stopper \\
	3\d\ra  & 19\rplus, 4\rplus\s, \d-Splinter \\
	3\h\ra  & 19\rplus, 4\rplus\s, \h-Splinter \\
	3\s     & 16-18, 4\rplus\s \\
	3SA     & to play, basierend auf semi-forcing in \c{} mit \s-Kürze \\
	4\c     & Schlemminteresse, 6\rplus\c{} 4\s \\
	4\s     & 10-12, 6\rplus\c{} 5\s
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 1\s\ra} \label{subsubsec:1c1H1S}

\begin{bids}
	Pass	& erlaubt (Plicht mit 5\rminus{} Punkten) \\
	1SA		& to play \ensuremath{\rightarrow} 2\s{} hatte 4er\s \\
	2\c\ra  & maximal inv Relay \ensuremath{\rightarrow} \ref{subsubsec:1c1H1S2c} \\
	2\d\ra  & gf, 5er\s \\
	2\h\ra  & gf, 5er\s{} 5er\h \\
	2\s     & 5er\s{}, Mini-Einladung (nur gegenüber 4er\s{} einladend) \\
	2SA\ra  & Relay zu 3\c{} \ensuremath{\rightarrow} \\
        	& \subbids{
	    	pass & wollte 3\c{} spielen \\
	    	Rest & SI-\c} \\
	3\m     & INV 5\s{} 5\m{} \ensuremath{\rightarrow} pass verneint 3er\s \\
	3\h\ra  & Splinter, 6er\s \\
	3\s     & gf, 6er\s \\
	3SA		& to play \\
	4\m		& Splinter, 6er\s \\
	4\s		& to play
\end{bids}

\subsubsection{1\c\rs1\h\ra1\s\ra2\c\ra} \label{subsubsec:1c1H1S2c}

\begin{bids}
	2\d\ra	& kein 4er\h \\
	& \subbids{
		Pass	& will jetzt 2\d{} spielen \\
		2\h		& inv, 5er\h \\
		2\s		& inv, 5er\h{} 4er\s \\
		2SA		& inv, 4\rplus\h \\
		3\m		& inv, 4er\h{} 5\rplus\m \\
		3\h		& inv, 6er\h} \\
	2\h\ra	& 4er\h
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 1SA\ra} \label{subsubsec:1c1H1SA}

Eröffner hat entweder einen schwachen Einfärber mit 3er\s{} oder einen starken Einfärber mit oder ohne 3er\s{}.

\begin{bids}
	Pass	& verboten \\
	2\c		& to play gegenüber schwachem Einfärber \ensuremath{\rightarrow} \\
	& \subbids{
		Pass 	& schwache Variante \\
		Rest 	& starke Variante, \tbd{}} \\
	2\d\ra	& inv+ Relay \ensuremath{\rightarrow} \\
	& \subbids{
		2\s 	& schwache Variante \ensuremath{\rightarrow} Weiterreizung natürlich (Gebote unterhalb 3SA sind einladend) \\
		Rest	& starke Variante, jetzt gf, so natürlich wie möglich} \\
	2\s 	& to play gegenüber schwachem Einfärber \ensuremath{\rightarrow} \\
	& \subbids{
		Pass 	& schwache Variante \\
		Rest 	& starke Variante, \tbd{}} \\
	Rest	& \tbd{}
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 2\c\ra} \label{subsubsec:1c1H2c}

\begin{bids}
	Pass	& erlaubt \\
	2\d\ra	& \tbd{INV+ 3.FF (hat kein 4\rplus\h) \ensuremath{\rightarrow}} \\
	& \subbids{
		2\h 	& rundenforcierend, 5\rplus\c{} 4\h{} \ensuremath{\rightarrow} \\
		& \subbids{
				Pass	& verboten \\
				3\d		& gf, 4.FF \\
				<3\d	& inv, so nat wie möglich \\
				>3\d	& gf, so nat wie möglich} \\
		2\s		& min, ohne \h-Stopper, mit Double Figur in \s \\
		2SA		& min, mit \h-Stopper \\
		3\c 	& min, ohne \h-Stopper, ohne Double Figur in \s \\
		3\d 	& max (gf), 5\rplus\c{} 4\d{}, mit oder ohne \h-Stopper \\
		3\h\ra 	& max (gf), ohne \h-Stopper, ohne Double Figur in \s \\
		3\s		& max (gf), ohne \h-Stopper, mit Double Figur in \s \\
		3SA 	& max, 6\rplus\c{} mit \h-Stopper} \\
	2\h		& gf, 5+\s{} 4+\h \\
	2\s		& inv, 6er\s \\
	2SA		& inv \\
	3\c		& inv \\
	3\d 	& gf, 5+\s{} 5+\d \\
	3\h		& gf, 5\s{} 5\h \\
	3\s		& gf, 6er\s \\
	3SA		& to play \\
	4\s		& to play
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 2\d} \label{subsubsec:1c1H2d}

\begin{bids}
	Pass	& erlaubt (nur mit 3-5 und nur 4er\h{} und 3\rplus\d) \\
	2\h\ra	& gf, 4.FF \\
	2\s 	& 5er\s (3\rplus), rundenforcierend \ensuremath{\rightarrow} \\
			& \subbids{
			2SA		& nf (16-18), 2\rminus\s{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
			3\c		& nf (16-18), 2\rminus\s{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
			3\h\ra	& gf (19\rplus), 4.FF, 2\rminus\s{} \ensuremath{\rightarrow} nat \\
			3\s		& nf (16-18), 3er\s{} \ensuremath{\rightarrow} alles außer pass ist gf \\
			4\h		& gf (19+), Splinter, 3er\s \\
			4\s		& 19+, 3er\s} \\
	2SA		& Relay (entweder 6-7 oder SI in \c) \\
			& \subbids{
			3\c		& 16-18 \ensuremath{\rightarrow} pass und 3\d sind to play, Rest SI in \c \\
			3\d		& gf, 6\rplus\c{} 5\rplus\d \\
			3\h\ra	& gf, 4.FF \\
			Rest	& gf und nat} \\
	Rest	& gf und nat
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 2\h} \label{subsubsec:1c1H2H}

\begin{bids}
	Pass	& erlaubt (nur mit 3-5 und nur 4er\h{} und 3\rplus\h) \\
	2\s 	& 5er\s (3\rplus), rundenforcierend \ensuremath{\rightarrow} \\
	& \subbids{
		2SA		& nf (16-18), 2\rminus\s{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
		3\c		& nf (16-18), 2\rminus\s{} \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
		3\d\ra	& gf (19\rplus), 4.FF, 2\rminus\s{} \ensuremath{\rightarrow} nat \\
		3\s		& nf (16-18), 3er\s{} \ensuremath{\rightarrow} alles außer pass ist gf \\
		4\d		& gf (19+), Splinter, 3er\s \\
		4\s		& 19+, 3er\s} \\
	2SA		& Relay (entweder 6-7 oder SI in \c) \\
	& \subbids{
		3\c		& 16-18 \ensuremath{\rightarrow} pass und 3\h sind to play, Rest SI in \c \\
		3\d\ra	& gf, 4.FF \\
		3\h		& gf, 6\rplus\c{} 5\rplus\h \\
		Rest	& gf und nat} \\
	3\d\ra	& gf, 4.FF \\
	Rest	& gf und nat
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 2\s} \label{subsubsec:1c1H2S}

\begin{bids}
	Pass	& erlaubt \\
	2SA\ra	& relay (insbes. mit SI anwenden) \\
	& \subbids{
		3\c\ra	& 3415 Typ \\
		3\d\ra	& 3145 Typ (selten, da Eröffner 1\d{} eröffnen konnte) \\
		& Auf 3\c{} / 3\d{} \ensuremath{\rightarrow} \\
		& \subbids{
			3\s			& inv \\
			4\s			& to play \\
			Kürze\ra	& SI-\s \\
			4\c			& \tbd{bedingte Assfrage} \\
			3SA			& to play \\
			4er-Farbe	& gf, Fit (gf, weil die Hände dann gut zusammenpassen)} \\
		3\h\ra	& max, 4er\s \\
		3\s\ra	& min, 4er\s \\
		3SA		& max, 4er\s{} und schlägt 3SA vor \\
		4\d/4\h	& Splinter, 4er\s{} und sehr gutes Maximum \\
		4\s		& max, 4er\s, sehr kontrollschwach} \\
	3\c		& inv, nf, 4er\s, 3\rplus Fit in \c \\
	3\d		& Long Suit Trial \d \\
	3\h		& Long Suit Trial \h \\
	3\s 	& inv, 5er\s \ensuremath{\rightarrow} ablehnen mit min und nur 3er\s \\
	3SA		& Spielvorschlag (Eröffner darf auch mit 4er\s passen) \\
	4\c		& Splinter (Chicane) \\
	4\d		& Splinter \\
	4\h		& Splinter \\
	4\s		& to play
\end{bids}

\subsubsection{1\c\rs{} 1\h\ra{} 2SA\ra} \label{subsubsec:1c1H2SA}

\begin{bids}
	3\s		& nf, hatte nur 3-5 Punkte \\
	3\c\ra	& gf ohne SI \\
	4\s		& gf ohne SI (0 Keycards) \\
	Rest	& gf, SI \tbd{Kürzen/Cuebids, je nach Vereinbarung}
\end{bids}

\subsection{1\c\rs{} 1\s\ra} \label{subsec:1c1S}

\begin{bids}
	Pass    & verboten \\
	1SA     & 12-14 balanced \ensuremath{\rightarrow} \\
			& \subbids{
				2\m	& to play \\
				2SA	& inv \\
				2\m	& inv} \\
	2\c     & a. 11-15, Zweifärber 5\rplus\c{} 4\rplus\d/\h/\s \\
	        & b. 11-15, Einfärber 6\rplus\c \ensuremath{\rightarrow} \\
	        & \subbids{
	        	2\d	& to play \\
	        	2SA & to play \\
	        	3\c & INV} \\
	2\d     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\d, forcing \ensuremath{\rightarrow} \\
			& \subbids{
				2\M\ra	& gf, Wert \\
				2SA\ra	& relay (6-7 Punkte) \ensuremath{\rightarrow} \\
						& \subbids{
							3\c\ra & 16-18 nf \ensuremath{\rightarrow} 3\d{} to play \\
							3\d\ra	& gf, 16\rplus{} 6\rplus\c{} 5\rplus\d \\
							3\M		& gf, 19\rplus{}, Wert \\
							3SA		& to play} \\
				3\m 	& gf, fit \\
				3\M\ra	& Splinter mit \d-Fit \\
				3SA		& to play} \\
	2\M     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\M, forcing \ensuremath{\rightarrow} \\
			& \subbids{
				2oM		& gf, Wert \\
				2SA\ra	& relay (6-7 Punkte) \ensuremath{\rightarrow} \\
						& \subbids{
							3\c\ra & 16-18 nf \ensuremath{\rightarrow} 3\d{} to play \\
							3\M\ra	& gf, 16\rplus{} 6\rplus\c{} 5\rplus\M \\
							3\d		& gf, 19\rplus{}, Wert \\
							3oM		& gf, 19\rplus{}, Wert \\
							3SA		& to play} \\
				3\c 	& gf, fit \\
				3x ohne Sprung	& Wert, gf \\
				3x im Sprung\ra	& Splinter mit \c-Fit \\
				3SA		& to play} \\
	2SA\ra  & 16\rplus, starker Einfärber mit 6\rplus\c, rundenforcierend \ensuremath{\rightarrow} \\
			& \subbids{
				3\c		& nf, einziger möglicher Teilkontrakt \\
				Rest	& gf, Stopper} \\
	3\c     & 13-15, 7\rplus\c{} \ensuremath{\rightarrow} alles außer pass ist gf, neue Farben zeigen Stopper \\
	3x      & gf, 6\rplus\c{} 5\rplus x
\end{bids}

\subsection{1\c\rs{} 1SA} \label{subsec:1c1SA}

\begin{bids}
	Pass    & erlaubt (Pflicht mit 12-14 bal) \\
	2\c     & a. 11-15, Zweifärber 5\rplus\c{} 4\rplus\d/\h/\s \\
	        & b. 11-15, Einfärber 6\rplus\c \\
	        & \ensuremath{\rightarrow} nat \\
	2\d     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\d, gameforcing \ensuremath{\rightarrow} \M{} zeigen Werte \\
	2\M     & 16\rplus, starker Zweifärber 5\rplus\c{} 4\rplus\M, gameforcing \M{} zeigen Werte \\
	2SA\ra  & 16\rplus, starker Einfärber mit 6\rplus\c, gameforcing \ensuremath{\rightarrow} neue Farben zeigen Stopper \\
	3\c     & 13-15, 7\rplus\c{} \ensuremath{\rightarrow} alles außer pass ist gf, neue Farben zeigen Stopper \\
	3\d		& 13-15 6\rplus\c{} 5\rplus\d \ensuremath{\rightarrow} \\
			& \subbids{
				Pass	& erlaubt \\
				4\c		& to play \\
				3\h		& SI-\c \\
				3\s		& SI-\d} \\
	3\M     & 13-15 6\rplus\c{} 5\rplus\M \ensuremath{\rightarrow} \\
			& \subbids{
				Pass		& erlaubt \\
				4\c			& to play \\
				neue Farbe	& SI-\c} \\
	3SA     & to play \\
	4\c		& \tbd{bedingte Assfrage}
\end{bids}

\subsection{1\c\rs{} 2\c\ra} \label{subsec:1c2c}

\begin{bids}
	Pass    & verboten \\
	2\d\ra  & 12-14 balanced oder 4414 \\
	2\h     & a. 11\rplus, 5\rplus\c{} 4\rplus\h \\
        	& b. 15\rplus, 4414 \\
	2\s     & 11\rplus, 5\rplus\c{} 4\rplus\s \\
	2SA\ra  & 11\rplus, 5\rplus\c{} 4\rplus\d \\
	3\c     & 11\rplus, 6\rplus\c{} Einfärber
\end{bids}

\subsection{1\c\rs{} 2\d\ra} \label{subsec:1c2d}

\begin{bids}
	Pass    & nicht empfohlen \\
	2\h     & 11-17, 3\rplus\h \ensuremath{\rightarrow} \\
			& \subbids{
				Pass	& 3-8(9) \\
				2\s		& 9-11, 6\rplus\s{} 4er\h \\
				2SA		& 10-11, 5422 \\
				3\c		& 9-11, 5413 \\
				3\d		& 10-11, 5431 \\
				3\h		& 9-11, 55MM} \\
	2\s     & 11-17, 2\rplus\s \\
			& \subbids{
				Pass	& 3-8(9) \\
				2SA		& 10-11, 5422 \\
				3\c		& 9-11, 5413 \\
				3\d		& 10-11, 5431 \\
				3\h		& 9-11, 55MM \\				
				3\s		& 9-11, 6\rplus\s{} 4er\h} \\
	2SA     & 17-19 einladend, max. 2er\s{} und 3er\h{} \ensuremath{\rightarrow} \\
			& \subbids{
				Pass	& erlaubt, 3-6 \\
				3\c		& 3-6, 3\rplus\c, lehnt ab \\
				3\d		& 7\rplus, 4.FF, nimmt an \ensuremath{\rightarrow} \\
						& \subbids{
							3\h & 3er\h \\
							3\s & 2er\s} \\
				3\h		& 5-6, 55MM, nf \\
				3\s		& 5-6, 6\rplus\s{} 4er\h, nf \\
				3SA		& 7\rplus, to play} \\
	3\c     & to play, 6\rplus\c{} (Antworter soll nicht einladen)\\
	3\d\ra  & 4.FF gf \ensuremath{\rightarrow} \\
			& \subbids{
				3\h 	& 55MM \\
				3SA		& was in \d \\
				3\s		& sonst} \\
	3\h     & 16-18, 4\rplus\h, einladend \\
	3\s     & 16-18, 3\rplus\s, einladend \\
	3SA     & to play \\
	4\M     & to play
\end{bids}

\subsection{1\c\rs{} 2\M} \label{subsec:1c2M}

\begin{bids}
	Pass	& erlaubt \\
	3\M		& 3\rplus\M, schwache INV \ensuremath{\rightarrow} Antworter darf mit gutem Max heben \\
	4\M		& to play (evtl. nur nach Law) \\
	2SA\ra	& inv\rplus{} Relay, verspricht 1\rplus\rplus\M \ensuremath{\rightarrow} \\
			& \subbids{
				3\c\ra	& 3-4 oder 7-8 und schlechte Farbe \ensuremath{\rightarrow} Falls Eö 3\M{} reizt, reizt man mit 7-8 3SA \\
				3\d\ra	& 5-6, schlechte Farbe \\
				3\h\ra	& 5-6, gute Farbe \\
				3\s\ra	& 7-8, gute Farbe \\
						& gute Farbe hat mind. 5 Punkte in der Farbe} \\
	3\c						& to play \\
	neue Farbe ohne Sprung	& rundenforcierend, einziger möglicher TK ist 3\M \\
	neue Farbe im Sprung\ra	& \M-Fit, Splinter \\
	3SA						& to play \\
\end{bids}

\section{1\c\rs{} mit Gegenreizung} \label{sec:1c mit Gegenreizung}

Nach 1\c\rs{} (1\d/\h) ist X das \glqq gestohlene Gebot\grqq, transferiert also auf \h/\s.

\chapter{1\d- Eröffnung} \label{ch:1d}

Hände, die man mit 1\d{} eröffnet, fallen grob in zwei Kategorien: Balanced oder Unbalanced.

Alle Balanced Hände mit 18-20 Punkten und ohne 5er-Oberfarbe werden mit 1\d{} eröffnet, unabhängig davon, welche Unterfarbe länger ist.
Wenn man 1\d{} eröffnet und im Rebid eine Balanced Hand zeigt, hat man immer 18-20 Punkte,
andere Punktespannen für Balanced Hände werden stattdessen 1\c, 1SA, 2SA oder 2\c{} eröffnet.

Unbalanced Hände, die mit 1\d{} eröffnet werden sind entweder \d-Einfärber, Zweifärber mit \d{} als längste Farbe, oder 4441-Hände mit 4er\d.

\section{Antworten auf 1\d\rs} \label{sec:1d}

Die Antworten 2\c{} und 2\d{} sind künstlich. 2\c\ra{} ist ein gameforcing Relay, 2\d\ra{} zeigt 5\rplus\s{} 4\rplus\h.

Prioritäten bei den Antworten:
\begin{itemize}
	\item Mit gf Händen ohne 5er-OF reizt man 2\c\ra{} (4er-OF zeigt man später)
	\item Mit 44 in den Oberfarben ohne gf reizt man 1\h
	\item Mit 5\rplus\s{} 4\rplus\h{} antwortet man ohne gf 2\d\ra, mit gf 1\s
\end{itemize}

\tbd{Alternative: Bei gf Hände mit 4er-Oberfarben diese immer auf 1er-Stufe reizen, 2\c{} verneint dann 4er-Oberfarbe}

Antworten auf 1\d\rs:
\nopagebreak

\begin{bids}
	Pass    & 0-5 \\
	1\h     & 3\rplus, 4\rplus\h \\
	        & mit 3-5 sollte man 5er\h{} haben \\
        	& mit gf und nur 4er\h{} stattdessen 2\c\ra{} reizen \\
        	& \ensuremath{\rightarrow} \ref{subsec:1d1H} \\
	1\s     & 3\rplus, 4\rplus\s \\
	        & mit 3-5 sollte man 5er\s{} haben \\
	        & mit gf und nur 4er\s{} stattdessen 2\c\ra{} reizen \\	        
	        & \ensuremath{\rightarrow} \ref{subsec:1d1S} \\
	1SA     & 6-11 keine 4\rplus\M, bal o. unbal. Notgebot \ensuremath{\rightarrow} \ref{subsec:1d1SA} \\
	2\c\ra  & gf Relay, keine 5\rplus\M \ensuremath{\rightarrow} \ref{subsec:1d2c} \\
	2\d\ra  & 3-11, 5\rplus\s{} 4\rplus\h \ensuremath{\rightarrow} \ref{subsec:1d2c} \\
	2\M     & schwach, 6\rplus\M \ensuremath{\rightarrow} \ref{subsec:1d2M} \\
	2SA     & 11-12, 4333 ohne 4er OF \ensuremath{\rightarrow} \\
			& \subbids{
				3\d & to play \\
				Rest & gf} \\
	3\m\ra  & 9-11, 6\rplus\m{} (mind. KB10xxx), ohne 4er OF \\
			& \ensuremath{\rightarrow} alles außer pass ist gf \\
	3\M     & schwach, 7\rplus\M \\
	3SA     & 13-15, 4333 ohne 4er \M
\end{bids}

\subsection{1\d\rs{} 1\h} \label{subsec:1d1H}

Nach einer 1\M{}-Antwort reizt der 1\d-Eröffner mit der Balanced Hand immer 1SA. Im Vergleich zur 1\c-Eröffnung steht im Rebid 1SA daher nicht als künstliches Gebot zur Verfügung.
Jetzt werden 2SA\ra{} und 3\d\ra{} verwendet, um starke Einfärber zu zeigen. Dies bedeutet wiederum, dass man 4\M{} und 3SA\ra{} nun als starke Hebungen der Oberfarben benötigt. 3\d\ra{} und 4\M{} sind demnach nicht auf extreme Verteilungen beschränkt und 3SA\ra{} ist nicht to play.

\begin{bids}
	Pass    & verboten \\
	1\s     & 11\rplus, 4\rplus\rplus\d{} 4\rplus\s, fast nie zu passen \ensuremath{\rightarrow} \ref{subsubsec:1d1H1S} \\
	1SA     & 18-20 balanced, 2-4\h \ensuremath{\rightarrow} \ref{subsubsec:1d1H1SA} \\
	2\c\rka & a. 11-18, 5\rplus\d{} 4\rplus\c \\
			& b. 11-15, 5\rplus\c{} 4\rplus\d  \ensuremath{\rightarrow} \ref{subsubsec:1d1H2c} \\
	2\d     & 11-15, Einfärber 6\rplus\d \ensuremath{\rightarrow} \ref{subsubsec:1d1H2d} \\
	2\h     & 11-15, 3\rplus\h{} (3er fit kann nur eine 5431-Hand sein, meist 4er) \ensuremath{\rightarrow} \ref{subsubsec:1d1H2H} \\
	2\s\rka & 10\rplus, 6\rplus\d{} 5\rplus\s \ensuremath{\rightarrow} \ref{subsubsec:1d1H2S} \\
	2SA\ra  & a. 19\rplus, 6\rplus\d, Einfärber \\
	        & b. 16-18, 6\rplus\d, Einfärber mit 3er\h \ensuremath{\rightarrow} \ref{subsubsec:1d1H2SA} \\
	3\c     & 19\rplus, 5\rplus\d4\rplus\c gf. \ensuremath{\rightarrow} \ref{subsubsec:1d1H3c} \\
	3\d\rka & 16-18, 6\rplus\d, Einfärber mit max. 2er\h \ensuremath{\rightarrow} alles außer pass ist nat und gf \\
	3\h     & 16-18, 4\rplus\h \\
	3\s\ra  & 19\rplus, 4\rplus\h, \s-Splinter \\
	4\c     & 19\rplus, 4\rplus\h, \c-Splinter \\
	3SA\ra  & 22\rplus, 4\rplus\h \\
	4\d     & Schlemminteresse, 6\rplus\d{} 4\h \\
	4\h     & 19-21, 4\rplus\h
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 1\s} \label{subsubsec:1d1H1S}

Hier ist erneut Two-Way-Checkback an. Wieder ist zu beachten, dass Eröffner eine größere Range an Punkten und Verteilungen haben kann. Daher darf Eröffner nach 1\d\rs{} 1\h{} 1\s{} 2\c\ra{} aus der Sequenz ausbrechen, wenn er eine Hand hat, mit der er sehr unglücklich wäre, wenn Responder 2\d{} passt \tbd{in etwas geringerem Ausmaß gilt gleiches auch für 1\d\rs{} 1\h{} 1\s{} 2SA\ra}.

\begin{bids}
	Pass	& erlaubt (sollte 6\rminus{} Punkte haben) \\
	1SA		& 6-10, natürlich \\
	2\c\ra	& maximal inv Relay \ensuremath{\rightarrow} \ref{subsubsec:1d1H1S2c} \\
	2\d\ra	& gf, 5er\h \\
	2\h		& inv, 6er\h \\
	2\s		& 6-10 FV, 4er\s \\
	2SA\ra  & Relay zu 3\c{} \ensuremath{\rightarrow} \\
			& \subbids{
				pass & wollte 3\c{} spielen \\
				Rest & SI-\c} \\
	3\c		& inv, 5er\h{} 5er\c \\
	3\d		& \tbd{gf, \d-Fit?}\\
	3\h		& gf, 6er\h \\
	3\s		& \tbd{gf, \s-Fit?} \\
	4\m		& Splinter, mit \s-Fit (AUCH 4\d{} !!!) \\
	4\h		& knappes gf, 7\rplus\h \\
	4\s		& Spielvorschlag (max. 1 Keycard ohne \s-Dame)
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 1\s{} 2\c\ra} \label{subsubsec:1d1H1S2c}

\begin{bids}
	2\d\ra	& Normalgebot \\
			& \subbids{
				Pass	& will jetzt 2\d{} spielen \tbd{sollte das hier erlaubt sein?} \\
				2\h		& inv, 5er\h \\
				2\s		& inv, \s-Fit \\
				2SA		& inv, 4\rplus\h \\
				3\c		& inv, 4er\h{} 5er\c \\
				3\d		& inv, \d-Fit} \\
	Anderes	& \tbd{Ausbrechen mit viel Verteilung/Punkten?}
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 1SA\ra} \label{subsubsec:1d1H1SA}

Eröffner hat 18-20 balanced und kann 2-4 \h s haben. Jetzt ist wieder einmal 2-Way-Checkback an.

\begin{bids}
	Pass	& erlaubt \\
	2\c\ra  & maximal inv Relay \ensuremath{\rightarrow} \ref{subsubsec:1d1H1SA2c} \\
	2\d\ra  & gf, 4\rplus\h \\
			& \subbids{
				2\h & 3er\h \\
				3\h & 4er\h} \\
	2\h     & to play 5er\h{} \ensuremath{\rightarrow} Hebung zeigt 4er\h \\
	2\s     & inv, 4\h{} 4\s \\
	2SA\ra  & Relay zu 3\c{} \ensuremath{\rightarrow} \\
			& \subbids{
				pass & wollte 3\c{} spielen \\
				Rest & SI-\c} \\
	3\m     & inv 5\h{} 5\m{} \ensuremath{\rightarrow} pass verneint 3er\h \\
	3\h     & gf, 6er\h \\
	3\s\ra  & Splinter, 6er\h \\
	3SA		& to play \\
	4\m		& Splinter, 6er\h \\
	4\h		& to play
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 1SA\ra{} 2\c\ra} \label{subsubsec:1d1H1SA2c}

\begin{bids}
	2\d\ra	& kein 4er\h \\
	& \subbids{
		Pass	& will jetzt 2\d{} spielen \\
		2\h		& inv, 5er\h \\
		2\s		& inv, 5er\h{} 4er\s \\
		2SA		& inv, 4\rplus\h \\
		3\m		& inv, 4er\h{} 5\rplus\m \\
		3\h		& inv, 6er\h} \\
	2\h\ra	& 4er\h
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 2\c\rka} \label{subsubsec:1d1H2c}

\begin{bids}
	Pass	& erlaubt (bei 3-7 Punkten reizen, wenn irgendwie sinnvoll, mit 8-9 möglichst vermeiden) \\
	2\d		& 3-9, ausbessern (bei 8-9 und 2er\d{} 4er\c{} ist das besser als Pass) \ensuremath{\rightarrow} mit 16-18 weiterreizen \\
	2\h		& inv, 6\rplus\h \\
	2\s		& gf, 4.FF \\
	2SA		& inv \\
	3\m		& inv (3\c{} kann etwas schwächer sein als 3\d) \\
	3\h		& gf, 6\rplus\h \\
	3SA		& to play \\
	4\m		& \tbd{Bedingte Assfrage} \\
	4\h		& to play
\end{bids}

\subsubsection{1\d\rs{} 1\h{} 2\d} \label{subsubsec:1d1H2d}



\subsubsection{1\d\rs{} 1\h{} 2\h} \label{subsubsec:1d1H2H}



\subsubsection{1\d\rs{} 1\h{} 2\s} \label{subsubsec:1d1H2S}



\subsubsection{1\d\rs{} 1\h{} 2SA\ra} \label{subsubsec:1d1H2SA}



\subsubsection{1\d\rs{} 1\h{} 3\c} \label{subsubsec:1d1H3c}



\subsection{1\d\rs{} 1\s} \label{subsec:1d1S}

\begin{bids}
	Pass    & verboten \\
	1SA     & 18-20 balanced, 2-4\s \\
	2\c     & a. 11-18, 5\rplus\d{} 4\rplus\c \\
	        & b. 11-15, 5\rplus\c{} 4\rplus\d \\
	2\d     & a. 11-15, Einfärber 6\rplus\d \\
			& b. 11-15, 5\rplus\d{} 4\rplus\h \\
	2\h     & 16\rplus, 5\rplus\d{} 4\rplus\h \\
	2\s     & 11-15, 3\rplus\s{} (3er fit kann nur eine 5431-Hand sein, meist 4er) \\
	2SA\ra  & a. 19\rplus, 6\rplus\d, Einfärber \\
	        & b. 16-18, 6\rplus\d, Einfärber mit 3er\s \\
	3\c     & 19\rplus, 5\rplus\d{} 4\rplus\c{} gf. \\
	3\d\rka & 16-18, 6\rplus\d, Einfärber mit max. 2er\s \\
	3\s     & 16-18, 4\rplus\s \\
	3\h\ra  & 19\rplus, 4\rplus\s, \h-Splinter \\
	4\c     & 19\rplus, 4\rplus\s, \c-Splinter \\
	3SA\ra  & 22\rplus, 4\rplus\s \\
	4\d     & Schlemminteresse, 6\rplus\d{} 4\s \\
	4\s     & 19-21, 4\rplus\s
\end{bids}

\subsection{1\d\rs{} 1SA} \label{subsec:1d1SA}

\begin{bids}
	Pass    & erlaubt insbes. mit 4441 und \c-Kürze mit 11-15 \\
	2\c\ra  & 16\rplus, Zweifärber 5\rplus\d{} 4\rplus\c{} ((41)44 möglich?)\\
	2\d     & 11-15, 5\rplus\d{}  ohne 4\rplus\c \\
	2\h     & 16\rplus, 5\rplus\d{} 4\rplus\h{} (4-4-4-1 möglich) \\
	2\s     & 16\rplus, 5\rplus\d{} 4\rplus\s \\
	2SA\ra  & 19\rplus, 6\rplus\d, gameforcing  \\
	3\c 	& 11-15, 5\rplus4\rplus\m\m{} ((41)44 möglich) \\
	3\d		& 16-18, 6\rplus\d \\
	3\M     & 6\rplus\d{} 5\rplus\M, gf \\
	3SA     & to play
\end{bids}

\subsection{1\d\rs{} 2\c\ra} \label{subsec:1d2c}

\begin{bids}
	Pass    & verboten \\
	2\d\ra  & 18-20 balanced \\
	2\h     & 11\rplus, 5\rplus\d{} 4\rplus\h oder 4-4-4-1 / 1-4-4-4 \\
	2\s     & 11\rplus, 5\rplus\d{} 4\rplus\s oder 4-1-4-4 \\
	2SA\ra  & 11\rplus, 5\rplus4\rplus\m\m \\
	3\c     & 11\rplus, 6\rplus\d{} Einfärber \\
	3\d		& 16\rplus, 6\rplus\d{} Einfärber (sehr gute Farbe)
\end{bids}

\subsection{1\d\rs{} 2\d\ra} \label{subsec:1d2d}

\begin{bids}
	Pass    & erlaubt \\
	2\h     & 11-17, 3\rplus\h \\
	2\s     & 11-17, 2\rplus\s \\
	2SA     & 17-19 einladend, max. 2er\s{} und 3er\h{} \\
	3\c\ra  & 4.FF gf \\
	3\h     & 16-18, 4\rplus\h, einladend \\
	3\s     & 16-18, 3\rplus\s, einladend \\
	3SA     & to play \\
	4\M     & to play
\end{bids}

\subsection{1\d\rs{} 2\M} \label{subsec:1d2M}

\chapter{1\M-Eröffnung} \label{ch:1M}

\section{Antworten auf 1\M} \label{sec:1M}

\section{1\M{} mit Gegenreizung} \label{sec:1M mit Gegenreizung}

\begin{itemize}
	\item \tbd{Transfers nach 1\M{} (X) ?}
	\item \tbd{Transfers nach 1\M{} (2\c) ?}
	\item \tbd{1\s{} (2\d) 2\h{} forcing oder non-forcing?}
\end{itemize}

\chapter{1SA-Eröffnung} \label{ch:1SA}

\section{Antworten auf 1SA\rs}

\begin{table}[h]
	\centering
	\label{1SA}
	\makebox[\textwidth][c]{
		\begin{tabular}{c | c c c}
			& schwach & einladend & gf+  \\[5pt]\hline\rule{0pt}{30pt}
			(3\rminus3\rminus) 	& \makecell{pass \\ Transfer auf 3m}
			& \makecell{2SA mit 5\rplus5\rplus mm \\ 2\s{} sonst}
			& \makecell{2SA mit 5\rplus5\rplus mm \\ 2\s{} geht \\ 3\c{} geht auch} \\[25pt]
			(43\rminus)	& \makecell{pass \\ 2\c{} mgl mit 5\rplus m \\ Transfer auf 3m}	
			& \makecell{2\c{} (fit: raise, kein fit: 2SA)} 
			& \makecell[c]{3SA \\ (41)44: 3M \\ 3\c{} ohne SI\\ 2\c{} \\2\s{} mit Kürze in M} \\[35pt]
			44			& \makecell{pass \\ 2\c{} mgl mit 5\rplus m \\ Transfer auf 3m} 	
			& \makecell{2\c{} (fit: raise, kein fit: 2SA)} 
			& \makecell{2\c{} (fit: other major für SI) } \\[30pt]
			(53\rminus) 	& Jacobi 
			& \makecell{Jacobi (dann Trnf mit 5m) \\ 2\c{} mit 5\s{}3\h{} oder 5\s{} ohne 5\rplus m} 	
			& \makecell{Jacobi (dann Trnf mit 4m/3SA/raise) \\ 2\s{} mit Kürze in M \\ 3SA} \\[30pt]
			(54) 		& Garbage Stayman	
			& \makecell{2\c{} (auf 2\d: 2\s{} oder 2SA\ra) \\ nicht Jacobi!} 
			& 2\c{} (auf 2\d: 3M in kurzem M) \\[25pt]
			55 		& Garbage Stayman 
			& 3\d 
			& 4\c \\[20pt]
			6\rplus	& Jacobi
			& Jacobi (rebid: 3M-1)
			& \makecell{Jacobi (rebid: 3M-1) \\ 4M-1}
		\end{tabular}
	}
	\caption{Übersichtstabelle für die Antworten auf 1SA}
\end{table}

\begin{bids}
	2\c\rs & Stayman \tbd{ab wie vielen Punkten?}\\
	2\d\rs & \h-Transfer \tbd{nicht mit inv+ 5+\h4\s} \\
	2\h\rs & \s-Transfer \tbd{nicht mit inv+ 5+\s4\h{} oder inv+ 5+\s ohne 5+m}\\
	2\s\ra & min oder max? oder \c-Transfer \\
	2SA\ra & \tbd{pick a minor oder \d-Transfer} \\
	3\c\ra & Frage nach 5er-OF \\
	3\d\ra & 55MM, inv+ \\
	3\M    & \tbd{(41)44} \\
	3SA    & to play \\
	4\c\ra & 55MM gf oder slamforcing \\
	4\d\ra & 6+\h \\
	4\h\ra & 6+\s \\	
\end{bids}


Siehe auch die Tabelle \ref{1SA}.

Was in der Tabelle als Transfer auf 3m steht ist je nach Situation 2\s{} mit Ziel 3\c{} oder 2SA pick a minor / Ziel 3\d.

Mit Schlemminteresse ist 3\c{} ist nicht so toll, da man eventuell keine Kontrollen mehr abfragen kann. Am liebsten wendet man es an mit genau partieforcierend und (34) oder (33), ansonsten ist 2\c{} angenehmer.

\newpage

\section{1SA{} 2\c{} Stayman}

Mit beiden Oberfarben zu viert reizt Eröffner 2\h. Die Antwort 2\s{} zeigt dann eine einladende Hand mit 5\s{} und 3\rminus\h.

Von Seite des Antwortenden reizen alle einladende Hände vom Typ (4x)(5y) ohne OF-fit einfach 3m. Einladende Hände mit 5\s{} geben immer 2\s. Als Antwort auf 2\d{} beinhaltet 2SA\ra{} die einladende 5\h4\s{}-Hand.

\begin{bids}
	2\d\ra & keine 4er M \\
	& \subbids{
		2\h & garbage Stayman 5+4+MM; kein invite \\
		2\s & inv 5\s{} mit oder ohne 4\h{} (aber keine 5\h)\\
		2SA\ra & inv (5\h{} 4\s{} ist möglich) $\rightarrow$ alles außer pass nimmt an \\
		& \subbids{3\h & max und 3er\h} \\
		3m 	& to play, 5+m \\
		3\h\ra & gf; 4\h{} 5+\s \\
		3\s\ra & gf; 4\s{} 5+\h  \\
		3SA & to play \\
		4m 	& gf; 44(41) cRKCB \\
		4M 	& to play mit 6er M
	} \\
	2M 	& 4+M \\
		& \subbids{
			2SA & inv mit 4M' \\
			3m	& einladend ohne fit mit 5+m \\
			3M	& inv mit 4+M \\
			3M'\ra	& SI in M (hat 4+4MM, da nicht 3\c gestartet) \\
			3SA	& to play
		}
\end{bids}


\section{1SA 2\d/2\h}

Hier zunächst für 1SA{} 2\d. Antworten der eröffnenden Hand:

\begin{bids}
	2\h & Normalgebot \\
	3\h & Minimum mit 4er\h{} (the law) \\
	2SA & Maximum mit 4er\h. Darauf ist 3\d{} retroforcing
\end{bids}

Antworten auf 1SA\rs2\d\rs2\h:

\begin{bids}
	pass 	& erlaubt \\
	2\s\ra	& invite 5\h, keine 5+m \\
	2SA\ra 	& Transfer, 5+\c{} oder SI \\
	3\c\ra 	& Transfer, 5+\d{} oder SI \\
	3\d\ra	& inv+ mit 6+\h \\
	3\h 	& gf 5332 Hand; Frage \h-Fit? Antworte positiv mit cue-bids \\
	3SA		& choice of games \\
	3\s, 4m	& Splinter, 6+\h
\end{bids}

Die Transfers äußern dabei
	\begin{itemize}
		\item invite 5\h{} 5m, 
		\item gf 5\h{} 5m, oder
		\item SI 5\h{} 4m.
	\end{itemize}
Antworten auf die Transfers:

\begin{bids}
	3\h 		& \h-fit; nun gf \\
	Ausführung  & kein \h-fit, minimum \\
				& \subbids{
					4m		& gf; 5+m \\
					1 step 	& SI 5\h{} 4+m. Eröffner kann mit 3SA ablehnen} \\
	3SA			& kein \h-fit, maximum
\end{bids}

Für 1SA{} 2\h{} ist alles genau analog. Die einladende Hand mit 5\s{} ohne 5+m hat nach dem Normalgebot kein rebid und ist deshalb in Stayman.


\section{1SA 2\s}

2\s{} fragt min oder max. Genauer kann es sein
\begin{itemize}
	\item einladend ohne 4+\M,
	\item gf mit einer Kürze in \M{} und Zweifel an 3SA, oder
	\item Transfer auf 3\c.
\end{itemize}
Die Antworten sind

\begin{bids}
	2SA\ra 	& min\\
	3\c\ra 	& max
\end{bids}

Nach 2SA ist 3\c{} to play, ansonsten alles genauso wie die Antworten auf 3\c:

\begin{bids}
	pass 	& wollte 3\c{} spielen \\
	3\d		& deckst du \h? \\
			& \subbids{
				3SA 	& jaa! \\
				3\h\ra 	& nein und 3\rminus\s \\
				3\s\ra	& nein und 4\rplus\s 
			} \\
	3\h\ra 	& deckst du \s? ohne 3er \h{} (also eine 6+m) \\
			& \subbids{
				3SA 	& jaa! \\
				3\s\ra	& nein :( $\rightarrow${} 4m oder 5m 
			} \\
	3\s\ra 	& deckst du \s? habe 3er \h \\
			& \subbids{
				3SA 	& jaa! \\
				4m 		& nein und 4+m \\
				4\h 	& nein und zähneknirschend 4+\h
			} \\
	3SA		& to play \\
	4m		& cRKCB 
\end{bids}


\section{1SA 2SA}

2SA fragt nach der Lieblingsunterfarbe des Eröffners oder möchte 3\d{} spielen. Die Antworten sind

\begin{bids}
	3\c & \c{} sind länger oder 33mm\\
	3\d & \d{} sind länger oder 44mm
\end{bids}

Darauf wird geantwortet mit

\begin{bids}
	3\d		& to play \\
	3M		& Stopperfrage (sonst spielen wir UF) \\
	4m		& cRKCB
\end{bids}


\section{1SA 3\c}
Das 3\c-Gebot fragt nach 5er \M{} und ist partie-forcing. Nicht anzuwenden mit 4\rplus4\rplus MM, da man die nicht zeigen kann.

\begin{bids}
	3\d\ra 	& keine 5er \M \\
			& \subbids{
				3\h & 4er \s \\
					& \subbids{
						3\s & fit und keine Ruine \\
						3SA & kein fit \\
						4\s & fit und Ruine}\\
				3\s	& 4er \h \\
					& \subbids{
						3SA & kein fit \\
						4m 	& max und fit, cue\\
						4\h & min und fit}\\
				3SA	& to play \\
				4m 	& cRKCB\\
				4SA & quantitativ
			} \\
	3M		& 5er M \\
			& \subbids{
				3SA	& to play\\
				4M 	& to play\\
				M'	& SI in M\\
				4SA	& quantitativ}
\end{bids}


\chapter{2\c-Eröffnung} \label{ch:2c}

Die 2\c\rs-Eröffnung ist die einzige starke Eröffnung außer 2NT und partieforcing,
mit Ausnahme der 23--24 balanced-Hände, die auch enthalten sind.
Nach Gegenreizung ist man jedenfalls immer im Forcing-Pass.
Der AW reizt meistens 2\d{}, was nichts aussagt, oder 2\h{} mit 5er.

\section{Wiedergebote auf 2\d}

Die Wiedergebote zeigen 5er-Farben auf der 2er-Stufe und 6er-Farben auf der 3er-Stufe.
Ausnahme: mit 5-5 in UF hat man kein anderes Gebot. Mit 5431 und langer UF im Zweifelsfall SA.
Um die beiden Punktspannen 23--24 und 25+ für Balanced-Hände auseinander zu halten,
ist 2\h{} vom Eröffner doppeldeutig (Kokish-Konvention).

\begin{bids}[lllX]
    2\c\rs  & 2\d\ra    & Wiedergebote: \\
            &           & 2\h\ra    & Kokish: natürlich oder balanced, gf \\
            &           & 2\s       & 5\rplus\s \\
            &           & 2NT       & 23--24 \\
            &           & 3m        & 6+m (zur Not 5, insbesondere mit 5-5 in UF) \\
            &           & 3NT\rka   & Zum Spielen mit stehender 7er-Farbe \\
\end{bids}

\subsection{Weiterreizung nach 2\h{}-Wiedergebot}

Der AW kann entweder ein 2\s{}-Relais abgeben, um die Doppeldeutigkeit aufzulösen,
oder eine eigene Farbe im Transfer reizen.
Dabei eine Besonderheit: Es gibt keinen Transfer auf \h{}. 
Mit \h{}s reizt man 2\s{}, wartet auf 2SA vom EÖ und reizt dann ganz normal Transfer.
Wenn der EÖ nicht 2SA sagt, sondern selber \h{}s hat, umso besser!
Daher ist 3\d{} Transfer auf \s{}. Das lässt dem EÖ den Platz, mit 3\h{} auf seinen \h{}s zu bestehen.

\begin{bids}[lllX]
    2\c\rs  & 2\d\ra    & 2\h\ra    & Antworten: \\
            &           &           & \subbids{
                                        2\s\ra  & Relais, oft mehr oder weniger balanced, oder lange \h{}s \\
                                        2NT\ra  & Transfer \c{} \\
                                        3\c\ra  & Transfer \d \\
                                        3\d\ra! & Transfer \s \\
                                    }
\end{bids}

\textbf{Nach 2\s{}-Relais}

\begin{bids}[llllX]
    2\c\rs  & 2\d\ra    & 2\h\ra    & 2\s\ra    & Wiedergebote: \\
            &           &           &           & \subbids{
                                                    2NT & 25\rplus{} balanced \\
                                                    \multicolumn{2}{@{}l}{Alles andere zeigt \h{}s:} \\
                                                    3\c\d\s & zweite Farbe \\
                                                    3\h     & Einfärber, gute Farbe \\
                                                    3NT\ra  & Einfärber, für NT geeignet \\
                                                } \\
\end{bids}

Eine Schwierigkeit ist, dass nach 3\h{} und 3NT kein Platz mehr ist, um unterschiedlich positive Fits zu zeigen:
Man kann nur auf 4\h{} heben (negativ, max.\ 1 Stich) oder ein Kontrollgebot abgeben (leicht positiv+).
Es gibt hier keine Verwechslungsgefahr mit einem natürlichen Gebot -- wenn der AW eine eigene Farbe zu reizen hat, muss er gleich transferieren.

\begin{tbdblock}
    Man kann nach dem 2\s{}-Relais 3\c{} und 3\h{} vom EÖ vertauschen, sodass mit
    Einfärbern mehr Platz bleibt, auf Kosten der \h{}-\c{}-Zweifärber.
    Oder 3\c{} doppeldeutig mit 3\d{}-Relais.
\end{tbdblock}

\textbf{Nach Transfers}

Der EÖ führt den Transfer mit der balanced-Hand aus und reizt natürlich mit einer \h{}-Hand.
Mit Fit kann er im Sprung heben, um die Trumpffarbe festzulegen, egal ob balanced oder \h{}s.
Eine „Wiederholung” von 3\h{} legt \h{} als Trumpf fest -- eine Farbe vom AW ist darauf ein Cuebid, selbst die Wiederholung seiner Transfer-Farbe.

\begin{bids}[lllX]
    2\c\rs  & 2\d\ra    & 2\h\ra    & Transfer-Antworten und Weiterreizung: \\
            &           &           & \subbids{
                                        2NT\ra  & Transfer \c{} \\
                                                & \subbids{
                                                    3\c\ra  & 25\rplus bal. \\
                                                    3\d\s   & zweite Farbe mit \h{}s \\
                                                    3\h     & Gute \h{}, legt Trumpffarbe fest \\
                                                    3NT     & nat. mit \h{}s (kein Fit, Stopper in Restfarben) \\
                                                    4\c     & Fit \\
                                                } \\
                                        3\c\ra  & Transfer \d \\
                                                & Weiterreizung analog (4\c = zweite Farbe) \\
                                        3\d\ra! & Transfer \s \\
                                                & Weiterreizung analog: \\
                                                & \subbids{
                                                    3\h     & Gute \h{}, legt Trumpffarbe fest \\
                                                    3\s\ra  & 25\rplus bal. \\
                                                    3NT     & nat. mit \h{}s \\
                                                    4\c\d   & zweite Farbe \\
                                                    4\s     & Fit, ermutigend, nonforcing \\
                                                    4NT     & Assfrage, bestätigt \s{}
                                                } \\
                                    } \\
\end{bids}

In der letzten Zeile ist 4NT Assfrage auch ohne vorherige Fitbestätigung:
Es gibt hier wenig Grund für ein quantitatives Gebot; mit ausgeglichenen Händen kann man
ja erst den Transfer ausführen.
4\s{} als forcing zu spielen klingt jedenfalls gefährlicher, auch wenn es nicht forcierend selten
gereizt und noch seltener gepasst wird.

\subsection{Weiterreizung nach natürlichen Wiedergeboten}

\textbf{Nach 2\s{}}

Anders als nach 2\h{} ist kein Platz mehr für Transfers, 2SA wird als abwartendes Relais benötigt.
Dafür kann der EÖ auch nicht mehr die balanced-Hand haben, wodurch die Weiterreizung ziemlich analog wird.
Dazu kommt nur die Möglichkeit für den AW, die \s{}s direkt zu heben, auf 3er-Stufe, 4er-Stufe, oder mit Splinter.

\begin{bids}[lllX]
    2\c\rs  & 2\d\ra    & 2\s       & Weiterreizung: \\
            &           &           & \subbids{ 
                                        2NT\ra  & Abwartend \\
                                        3\c\d\h & 5\rplus{}er, positive Hand \\
                                                & \subbids {
                                                    3\s & legt \s{} als Trumpf fest \\
                                                    4\h{} auf 3\h & Fit, ermutigend, nonforcing \\
                                                    4NT auf 3\h & Assfrage, bestätigt \h{} \\
                                                } \\
                                        \multicolumn{2}{@{}l}{Hebungen:} \\
                                        3\s     & min.\ leicht positiv (ab 1,5 Stichen) \\
                                                & \subbids {
                                                    3NT & non-serious -- so gerade eine GF-Eröffnung
                                                } \\
                                        4\c\d\h & Splinter: 4 Trümpfe, Single, ca.\ 1 nützliche Figur (idR 1 A/K, evtl.\ noch 1D dazu) \\
                                        4\s     & negativ (max.\ 1 Stich) \\
                                    } \\
\end{bids}

\tbd{Man kann 3SA als mittlere Hebung spielen, kein SI, aber auch kein totaler Schrott.
Dann werden 3\s{} und 4\s{} eindeutiger.}

\textbf{Nach 3UF}

Eine neue Farbe vom AW kann eine lange Farbe sein, oder auch nur ein Stopper um 3SA zu spielen -- letzeres v.A.\ mit schwachen Händen, die nicht am UF-Schlemm interessiert sind.
Wenn der EÖ kein besseres Gebot hat, kann er mit 3er-Anschluss heben; der AW korrigiert auf 5 in Eröffnerfarbe, wenn es keine lange Farbe war.

\section{Andere Antworten als 2\d}

\subsection*{2\h{}-Antwort}

2\h{} nimmt dem EÖ mit unausgeglichenen Verteilungen keinen Bietraum, daher sind die Anforderungen viel geringer, als bei den anderen Antworten:
Eine brauchbare 5er-Farbe in einer positiven Hand reicht.
Trotzdem ist der AW nicht gezwungen, ein 5er-\h{} zu zeigen, wenn er der Meinung ist, nach einer 2\d{}-Antwort besser dazustehen.
Die Weiterreizung ist natürlich und folgt den gleichen Prinzipien wie nach 2\d-Antwort.
2SA vom EÖ zeigt 23\rplus{} -- die Punktspanne wird also nicht mehr genauer aufgelöst, dafür hat der AW jetzt die 3er-Stufe zur Verfügung, um eine zweite Farbe zu zeigen.

\subsection*{Höhere Antworten}

Die Antworten ab 2\s{} haben ein erhöhtes Risiko, dass der EÖ seine Farbe eine Stufe höher zeigen muss.
Sie werden deshalb viel seltener und nur mit bestimmten Handtypen abgegeben:
\begin{enumerate}
\item Gute 5-5-Hände.
\item Einfärber mit sehr guten Farben, die schon gegenüber einer Single-Topfigur
stehend sind (6er mit drei Figuren, 7er mit zwei Topfiguren).
\end{enumerate}

In jedem Fall ist das Ziel, eine Fitfarbe für einen Schlemm zu finden, die man nach
2\d-Antwort nicht gefunden hätte: Mit 5-5-Händen kann man gegenüber einem Einfärber
einen 5-3-Fit finden, hätte nach 2\d{} aber nicht genug Platz, beide Farben zu reizen.
Mit sehr guten Einfärbern kann es auch gegenüber einer Kürze die beste Trumpffarbe sein,
aber man könnte die Farbe nach 2\d{} höchstens einmal reizen.

\begin{bids}[llX]
    2\c\rs  & 2\s\ra    & 5\rplus\s-5\rplus x oder Einfärber \\
            &           & \subbids{
                            2NT\ra  & Relais \\
                                    & \subbids{
                                        3x  & 5\rplus-5\rplus \\
                                        3\s & Einfärber \textrightarrow{} Cuebids \\
                                    } \\
                        } \\
            & 2NT\ra    & 5\rplus-5\rplus UF \\
            & 3\c\d     & Einfärber \\
\end{bids}

\section{Nach Gegenreizung}

Nach Gegenreizung fehlt der Bietraum, um die SA-Punktspannen genau aufzulösen.
Die Reizung ist deshalb einfach partieforcing.

\begin{tbdblock}
    Am besten haben wir allgemeine Regeln, wann X in forcierenden Situationen take-out
    ist und wann Strafe (z.B. 1\h{} (--) 2\c{} (4\d) X = ?).
    Auf niedriger Stufe finde ich Takeout logischer, um es dem Partner leichter zu 
    machen, wenn der andere Gegner hebt. Das ergäbe dann:
\end{tbdblock}

\subsection*{Reaktion vom Antwortenden auf Gegenreizung}

\begin{bids}[lllX]
    2\c\rs  & (2\s) & Neue Farbe & positiv, ordentliche Farbe \\
            &       & X         & positiv, take-out \\
            &       & p         & kein klares Gebot \\
            &       &           & \subbids{
                                    2NT     & bal.\ (im Prinzip), 23\rplus, forcing \\
                                    X       & kein Gebot (hier im Prinzip balanced ohne ausreichenden Stopper, auf höherer Stufe schwammiger) \\
                                    3NT     & zum Spielen \\
                                    3\s\ra  & Frage nach Stopper mit stehender Farbe \\
                                } \\
\end{bids}

Ein Ausspielkontra wird wie ein 2\c-Gebot behandelt:

\begin{bids}[lllX]
    2\c\rs  & (X)   & 2\d   & natürlich \\
            &       & XX    & positiv, take-out \\
            &       & p     & nichts zu sagen \\
            &       &       & \subbids{ 
                                XX & Kein natürliches Gebot \\
                                3\c & Frage nach Stopper \\
                            } \\
\end{bids}

\subsection*{Reaktion vom Eröffner nach Reizung vom rechten Gegner}

Im Prinzip ähnlich, wobei der Eröffner daran denken sollte, dass er erst mal passen kann,
wenn er sich über Fitfarbe oder Kontrakthöhe nicht sicher ist.

\begin{bids}[lllllX]
    2\c\rs  & (--)  & 2\d\ra    & (3\c) & X     & take-out, spielbereit in Restfarben \\
            &       &           &       & p     & abwartend \\
\end{bids}

\chapter{2\d-Eröffnung} \label{ch:2d}
Die 2\d-Eröffnung verspricht einen OF-Zweifärber (mindestens 54) unter Eröffnungsstärke. Da der Partner mit 3er OF zunächst nicht weiß, ob er einen Fit hat, sollte das Gebot in erster und zweiter Hand eher solide sein (ca 8-10 HP mit vernünftigen Oberfarben, oder mindestens 10 Karten in OF).

\section*{Antworten und Weiterreizung} \label{sec:2d}
Alle OF-Gebote sowie 3\c{} und 3SA sind to play. Passen um \d{} zu spielen ist auch möglich.
Aus 3SA kann EÖ mit 65 Händen rauslaufen: 4\c{} (6er \h) bzw. 4\d{} (6er \s).

Weitere Antworten:

\begin{bids}
2SA & Fragt. Rebids: \\
    & \subbids{
        3\c & 4-5 \\
        3\d & 5-4 \\
        3\h & 5-5 Minimimum \\
        3\s & 5-5 Maximum \\
    } \\
4\c & Schlemminteresse in \h \\
4\d & Schlemminteresse in \s \\
\end{bids}

Bei den beiden schlemminteressierten Geboten reizt EÖ mit Minimum 4 in besagter OF, mit Maximum das Zwischengebot.

\chapter{2OF-Eröffnung} \label{ch:2M}
Weak-Two. Antworten:

\begin{bids}
Neue Farbe & natürlich, forcing \\
Hebung & Abschluss \\
2SA & einladend+, fragt. Rebids: \\
& \subbids{
3OF & Minimum \\
3 sonst & Werte \\
3SA & keine Werte außerhalb \\
4F & Splinter (ggf \h für \s) \\
}\\
\end{bids}

\chapter{2SA-Eröffnung} \label{ch:2SA}

\chapter{3x-Eröffnung} \label{ch:3x}

\chapter{Schlemmreizung} \label{ch:Schlemmreizung}

\section{Cuebids und Last Train} \label{sec:Cuebids}

\begin{itemize}
	\item Cuebids zeigen Erst- oder Zweitrundenkontrolle in der Farbe.
	\item 3SA kann ein Ersatzcuebid sein, falls 3\s{} z.B. Non-Serious ist.
	\item 4OF-1 als Antwort auf ein 4OF-2 Cuebid/Splinter ist Last-Train (Kontrolle oder Extrastärke fehlt, um die Assfrage zu stellen).
\end{itemize}

\section{3M Modul} \label{sec:3M Modul}

\subsection{Non-Serious Step 1 (wenn beide unlimitiert sind)}

Wenn in einer gameforcing oder schlemmeinladenden Reizung ein Oberfarbenfit mit 3\M{} (3M) bestätigt wurde:

\begin{itemize}
	\item Step 1 (= das erste Gebot nach 3\M) ist Non-Serious, ein direktes Cuebid ist Serious.
	\item Falls \h{} die Trumpffarbe ist, ist 3\s\ra{} Non-Serious und 3SA\ra{} Serious mit \s-Cuebid.
	\item Falls \s{} die Trumpffarbe ist, ist 3SA\ra{} Non-Serious nur Semi-Forcing (Partner darf passen, wenn er lieber 3SA spielen will).
\end{itemize}

\subsection{No Shortage (serious) Step 1 (wenn mindestens einer limitiert ist.)}

Falls ein Reizer bereits stark limitiert ist (=Punkterange max 3F), so kann er nie serious sein.
Bedeutungen der Gebote vom limitierten Reizer:

\begin{bids}
	Step1   & Waiting ohne Kürze \\
	Cuebid  & Kürze \\
	4M      & Katastrophenblatt
\end{bids}

Vom nicht limitierten Reizer (mit limitiertem Partner) ist jeder Schlemmversuch serious.
Bedeutungen der Gebote vom nicht limitierten Reizer (mit limitiertem Partner):

\begin{bids}
	Step1   & serious ohne Kürze (oder will Kürze nicht zeigen) \\
	Cuebid  & Kürze \\
	4M      & to play
\end{bids}

No Shortage Step 1 entfällt, falls derjenige keine Kürze haben kann, dann ist 3SA Spielvorschlag.

\section{RKCB 1430} \label{sec:RKCB}

Bei gefundenem Oberfarbenfit / wenn ein Partner eine Oberfarbe als Trumpf etabliert hat, ist 4SA immer RKCB.

\begin{bids}
	Step1   & 1 oder 4 \\
	Step2   & 0 oder 3 \\
	Step3   & 2 (oder 5) ohne Trumpfdame \\
	Step4   & 2 (oder 5) mit Trumpfdame \\
	\tbd{}  & \tbd{Antworten mit Chicane}
\end{bids}

Nach RKCB fragen wir mit \hyperref[subsec:Spiral Scan]{Spiral Scan} weiter.

Bedeutung von 4SA in anderen Situationen:
\begin{itemize}
	\item Sprung in 4SA ohne etablierte Trumpffarbe ist i.d.R. quantitativ.
	\item 4SA ohne Sprung kann to play sein, wenn keine Oberfarbe als Trumpf etabliert wurde.
	\item In vielen Situationen in der kompetitiven Reizung / Gegenreizung ist 4SA pick a minor oder zeigt beide Unterfarben.
\end{itemize}

\subsection{Bedingte Assfrage} \label{subsec:Bedingte Assfrage}

\tbd{Wollt ihr das spielen?}

\subsection{Exclusion RKCB} \label{subsec:Exclusion RKCB}

\tbd{3014 statt 1430?}

\subsection{Spiral Scan} \label{subsec:Spiral Scan}

Nach der ersten Antwort werden ggf. die Trumpf Dame und dann die einzelnen Könige erfragt und gereizt.
\tbd{Bekannte Chicanes des Partners werden dabei ausgelassen, Singles nicht. Karten, die der Antwortende nicht haben kann, werden ebenfalls ausgelassen} \\
Nach Step1/2 ist das nächstmögliche Gebot, das nicht in Trumpffarbe ist, die Frage nach Trumpfdame. Mit dem niedrigsten mögliche Gebot in der Trumpffarbe verneint man.
\tbd{Trumpfdame ab 11?-Kartenfit dazulügen?; Reihenfolge der Könige; Positivantworten}